...
* I found an HTTP service on port 80 so I tried accessing the content on the browser. I got redirected to a domain _titanic.htb_ but I couldn't access it because it wasn't in my list of known hosts, so I added it to the _/etc/hosts_ file and retried to access the web, this time successfully reaching the content of a site for booking trips
...
* So to check how the petition was being sent and handled, I retried filling and submitting the form, this time intercepting the petition using [_FoxyProxy_](../../web-exploitation/tools-and-utilities.md#foxyproxy) and [_Burpsuite_](../../web-exploitation/tools-and-utilities.md#burp-suite). I noticed the site was sending a POST petition to a _/book_ route which could be interesting, and afterwards also sending a petition to a _/download_ route, specifying the JSON file to be downloaded
...
* As the ticket was invoking the file by its complete name, maybe was searching it directly in the file system of the server, so this could be vulnerable to a File Inclusion vulnerability. So for testing this, I put the typic _/etc/passwd_ route and saw it retrieved the content of the file, confirming it was vulnerable
...
* I listed all the table contents finding two rows each one for the users I had seen on the webpage and a lot of entries that I didn't understand at the time, but some of them seemed to be hashes. Also, one of the entries had the value _pbkdf2$50000$50_ that with a little research, I understood was an iterative hashing method and used a salt value
...
* I thought maybe some of the other entries of the row could give me more information related to this hash and how was being used, so I used the _PRAGMA_ command to get information about it and found two entries were giving me the hash for the password of the user and the respective salt being used
...
* So I tried using this to connect to the machine with the _developer_ user via the SSH protocol and fortunately, I logged in successfully. After that, I sanitized the terminal to work more comfortably with it
...
* So I went to the folder where the script was going to act and based on the repository created a _C_ script with a test payload using the `sudo id` command. Then I compiled it as the _libxcb.so.1_ library that _Magick_ was about to use and run the _magick_ command in the same form as the script. I got a notification about the user not being in the sudoers file but confirmed it worked by looking at the _metada.log_ where the results were supposed to be saved, having RCE as the _root_ user
...
* With this, I created a new file changing the payload to one that could help me to gain a Reverse Shell. I set a [_Netcat_](../../networks/tools-and-utilities.md#netcat) listener to receive the connection and established the payload indicating the IP of my machine and the port where I was listening. Then I did the compiling process and ran the _magick_ tool again, successfully gaining a shell as the _root_ user
